...
public class BookController {
    ...
    @GetMapping("/api/book")
    public List<BookDto> findBookByName(@RequestParam String name) {
        try {
            return bookRepository.findByNameContainingIgnoreCase(name).stream().map(BookDto::toDto)
                    .collect(Collectors.toList());
        } catch (Exception e) {
    ...
    public BookDto getInfoBook(@RequestParam Long bookId) {
        try {
            return prepareBookDto(bookId);
    ...
    public String removeBookById(@RequestParam Long bookId) {
        try {
            if (bookId == null)
    ...
    @GetMapping("/lists/info")
    public BookDto getInfo() {
        try {
