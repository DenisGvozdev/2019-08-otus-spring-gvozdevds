    ...
    private void addAclToDataBase(Book book) {
        ...
        try {
            if (book != null && book.getId() > 0) {
                ...
                long objectIdEdentity = book.getId();


                AclClass aclClass = aclClassRepository.findByClasss(ACL_CLASS_BOOK).orElse(null);
                long objectIdClass = (aclClass != null) ? aclClass.getId() : 0;

                AclSid aclSidRoleRead = aclSidRepository.findBySid("ROLE_READ").orElse(null);
                long sidRoleRead = (aclSidRoleRead != null) ? aclSidRoleRead.getId() : 0;

                AclSid aclSidAdmin = aclSidRepository.findBySid("admin").orElse(null);
                long sidAdmin = (aclSidAdmin != null) ? aclSidAdmin.getId() : 0;

                AclSid aclSidUser = aclSidRepository.findBySid("user").orElse(null);
                long sidUser = (aclSidUser != null) ? aclSidUser.getId() : 0;

                int maxAceOrder = aclEntryRepository.findMaxAceOrder();

                if (objectIdEdentity > 0 && objectIdClass > 0 && sidRoleRead > 0 && sidUser > 0) {
